...
void TrainBatch(AlexNet *alexnet, image *inputs, uint8 *labels, int batchSize)
{
	...
	int i = 0;
	// int right=0;
	// ofstream   myfile("alexnet_parallel_mnist_batch100_5000_training.txt",ios::out|ios::app);
	// start_timer_alexnet();
...
#pragma omp parallel for
	...
	// printf("right:%d\n", right);
	// stop_timer_alexnet();	
	// std::cout << "The time of a batch of 1000 image trainng of Alexnet parallel is " << std::chrono::duration_cast<std::chrono::milliseconds>(duration_alex).count() << std::endl;
