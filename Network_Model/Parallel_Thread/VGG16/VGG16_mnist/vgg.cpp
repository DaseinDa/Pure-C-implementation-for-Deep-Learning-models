...
//((double *)inerror)[x] += ((double *)outerror)[y] * weight[x][y]; Calculate the gradient of Loss for input layer
//wd[x][y] += ((double *)input)[x] * ((double *)outerror)[y];	The gradient of Loss for weight, use for weight update
...
static inline void load_input(Feature *features, image input)
{
	...
	FOREACH(j, sizeof(image) / sizeof(*input))
		FOREACH(k, sizeof(*input) / sizeof(**input))
	{
		layer0[0][j + PADDING][k + PADDING] = (input[j][k] - mean) / std;//标准化
	}
}
...
// 	}
// }
   	ofstream   myfile("vgg_parallel_mnist_batch1000_6000_training.txt",ios::out|ios::app);
	start_timer_vgg();
#pragma omp parallel for
	...
	stop_timer_vgg();
	std::cout << "The time of each batch=300 image trainng of VGG parallel is " << std::chrono::duration_cast<std::chrono::milliseconds>(duration_vgg).count() << std::endl;
